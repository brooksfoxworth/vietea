...
class CartManager {
    constructor() {
        this.cart = [];
        this.footerCartCount = document.getElementById('cart-item-count');
        this.headerCartCount = document.getElementById('cart-count');
    }
    ...
    /**
     * Update the cart count badges in both header and footer
     */
    updateCartCount() {
        const totalItems = this.cart.reduce((sum, item) => sum + item.quantity, 0);
        
        // Update footer cart count if it exists
        if (this.footerCartCount) {
            this.footerCartCount.textContent = totalItems;
            
            if (totalItems > 0) {
                this.footerCartCount.classList.remove('hidden');
            } else {
                this.footerCartCount.classList.add('hidden');
            }
        }
        
        // Update header cart count if it exists
        if (this.headerCartCount) {
            this.headerCartCount.textContent = totalItems;
            
            if (totalItems > 0) {
                this.headerCartCount.classList.remove('hidden');
            } else {
                this.headerCartCount.classList.add('hidden');
            }
        }
    }
